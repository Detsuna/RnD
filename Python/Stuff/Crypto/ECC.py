...
class EllipticCurve(ABC) :     
    ...
    class Point() : 
        Inf:EllipticCurve.Point = object()
        O:EllipticCurve.Point = object()
        ...
        def __eq__(self, other:object) -> bool : return (isinstance(other, EllipticCurve.Point) and self.x==other.x and self.y==other.y)
        def __hash__(self) -> int : return (self.x, self.y).__hash__()

        def __add__(self, other) -> EllipticCurve.Point : 
            if not isinstance(other, EllipticCurve.Point) : return NotImplemented
            if self==EllipticCurve.Point.O and other==EllipticCurve.Point.O : return EllipticCurve.Point.Inf
            return (self.Curve | other.Curve).CheckHasPoint(self).CheckHasPoint(other).Dot(self, other)
        def __radd__(self, other) -> EllipticCurve.Point : return self.__add__(other)
        def __iadd__(self, other) -> EllipticCurve.Point : return self.__add__(other)

        def __mul__(self, other) -> EllipticCurve.Point : 
            if not isinstance(other, int) : return NotImplemented
            return self.Curve.CheckHasPoint(self).Ladder(other, self)
        def __rmul__(self, other) -> EllipticCurve.Point : return self.__mul__(other)
        def __imul__(self, other) -> EllipticCurve.Point : return self.__mul__(other)        
    _ = Point.Inf = Point.O = Point()
    ...
    def GeneratePoints(self) -> list[Point] : 
        points = set([EllipticCurve.Point.O, EllipticCurve.Point.Inf])
        for x in range(self.Prime) : 
            for y in range(self.Prime) : 
                point = EllipticCurve.Point(x, y, self)
                try : 
    ...
    def Ladder(self, x:int, P:Point) -> Point : 
        if x==0 : return EllipticCurve.Point.Inf

        R0 = EllipticCurve.Point.Inf
        R1 = P
    ...
    def CompressPoint(self, D:Point) -> Point : return EllipticCurve.Point(x=D.x, y=(D.y & 1))
    @abstractmethod
...
class Weierstrass(EllipticCurve) :
    ...
    def CheckHasPoint(self, P:EllipticCurve.Point) -> EllipticCurve : 
        if (P==EllipticCurve.Point.O or P==self.G or ((P.y**2) % self.Prime == (P.x**3 + self.A*P.x + self.B) % self.Prime)) : return self
        else : raise ValueError("point not on curve")
    def Dot(self, P:EllipticCurve.Point=EllipticCurve.Point.O, Q:EllipticCurve.Point=EllipticCurve.Point.O) -> EllipticCurve.Point : 
        if P==EllipticCurve.Point.O and Q==EllipticCurve.Point.O : return EllipticCurve.Point.Inf
        elif P==EllipticCurve.Point.O : return Q
        elif Q==EllipticCurve.Point.O : return P
        ...
        else : return EllipticCurve.Point.Inf

        R = EllipticCurve.Point(Curve=self)
        R.x = (l**2 - P.x - Q.x) % self.Prime
        R.y = (l * (P.x - R.x) - P.y) % self.Prime
        return R
    def DecompressPoint(self, C:EllipticCurve.Point) -> EllipticCurve.Point : 
        D = EllipticCurve.Point(x=C.x,  y=self._SqrtMod(C.x**3 + self.A * C.x + self.B, self.Prime), Curve=self)
...
class Montgomery(EllipticCurve) : 
    ...
    def CheckHasPoint(self, P:EllipticCurve.Point) -> EllipticCurve : 
        if (P==EllipticCurve.Point.O or P==self.G or ((self.B*P.y**2) % self.Prime == (P.x**3 + self.A*P.x**2 + P.x) % self.Prime)) : return self
        else : raise ValueError("point not on curve")
    def Dot(self, P:EllipticCurve.Point=EllipticCurve.Point.O, Q:EllipticCurve.Point=EllipticCurve.Point.O) -> EllipticCurve.Point : 
        if P==EllipticCurve.Point.O and Q==EllipticCurve.Point.O : return EllipticCurve.Point.Inf
        elif P==EllipticCurve.Point.O : return Q
        ...
        return R
    def DecompressPoint(self, C:EllipticCurve.Point) -> EllipticCurve.Point : 
        D = EllipticCurve.Point(x=C.x,  y=self._SqrtMod((C.x**3 + self.A * C.x**2 + C.x) * pow(self.B, -1, self.Prime), self.Prime), Curve=self)
        if bool(C.y) != bool(D.y & 1) : D.y = self.Prime - D.y
        return D
...
class TwistedEdwards(EllipticCurve) : 
    ...
    def CheckHasPoint(self, P:EllipticCurve.Point) -> EllipticCurve : 
        if (P==EllipticCurve.Point.O or P==self.G or (self.A*P.x**2 + P.y**2) % self.Prime == (1 + self.B*P.x**2*P.y**2) % self.Prime) : return self
        else : raise ValueError("point not on curve")
    def Dot(self, P:EllipticCurve.Point=EllipticCurve.Point.O, Q:EllipticCurve.Point=EllipticCurve.Point.O) -> EllipticCurve.Point : 
        if P==EllipticCurve.Point.O and Q==EllipticCurve.Point.O : return EllipticCurve.Point.Inf
        elif P==EllipticCurve.Point.O : return Q
        ...
        return R
    def DecompressPoint(self, C:EllipticCurve.Point) -> EllipticCurve.Point : 
        D = EllipticCurve.Point(x=C.x,  y=self._SqrtMod((self.A*C.x**2 - 1)* pow(self.B*C.x**2 - 1, -1, self.Prime), self.Prime), Curve=self)
        if bool(C.y) != bool(D.y & 1) : D.y = self.Prime - D.y
        return D


if __name__ == "__main__" : 
    with Weierstrass(17, 0, 7, EllipticCurve.Point(15, 13), Order=18) as curve : 
        actual = curve.G
    ...
    with Montgomery(11, 5, 7) as curve : 
        ...
        n = 0
        for point in points : 
            if point==EllipticCurve.Point.O : continue
